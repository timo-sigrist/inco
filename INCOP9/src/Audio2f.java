...
public class Audio2f
{
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            raw = new RandomAccessFile("AudioFile2.wav", "rw");
        ...
        for (int i = 0; i < sampleRate*duration; i++)
        {
            ...
            double skalierungsfaktor = (Math.pow(2, 15) -1) /2;
            int frequenz = 440;
            int frequenz2 = 880;
            double sig = Math.sin((i * 2.0 * Math.PI * frequenz) / sampleRate);
            double sig2 = Math.sin((i * 2.0 * Math.PI * frequenz2) / sampleRate);
            double ton = skalierungsfaktor * (sig);
            double ton2 = skalierungsfaktor * sig2;
            writeSample(ton);
            writeSample(ton2);
